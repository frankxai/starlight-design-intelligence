...
The first Figma build is deliberately small:

1. Import only source-backed foundations from the central brand pack and local implementation contract.
...
The team should select a plan that supports the intended shared-library and variable-mode workflow before attempting published multi-brand libraries.
...
4. Generate tool-specific projections rather than copying values by hand.
...
3. Establish the generated Figma projection and Code Connect or equivalent code-mapping pipeline for FrankX as the first proof.
